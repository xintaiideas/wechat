function dealStaff() {
	...
	this.basicdata;
	this.tabledata;
	this.dataurl="kapai.php?method=getjson";
	this.changeurl="kapai.php?method=change";
	this.addurl="kapai.php?method=add";
	this.columnnum = 0;
	this.columns = [{
		data: 'kid'
	}, {
	...
	this.init = function() {
		...
		if(info==null){
			...
			$("#tittle").text(info.name+"管理");
		}
	this.dataurl="kapai.php?method=getjson"+"&gid="+this.id;
	this.changeurl="kapai.php?method=change"+"&gid="+this.id;
	this.addurl="kapai.php?method=add"+"&gid="+this.id;	
		addbtn("添加","add()");
		addhref("优先级管理","kapai.html?id="+this.id);
		...
		try {
			...
			for(var j=0;j<this.basicdata.length;j++){
				this.basicdata[j].cz=" ";
				if(this.basicdata[j].status==0){
					this.basicdata[j].state="未销售";
				}else{
					this.basicdata[j].state="已销售";
				}
			}
	...
	this.dealdata = function(row, data, index) {
		$('td', row).eq(4).html('<button   onclick="deal.changepage('+data.id +');change();" class="btn btn-info ">修改</button>&nbsp;<button   onclick="deal.del(' + data.id + ');" class="btn btn-info ">删除</button>&nbsp;');
	}
	this.draw = function() {
		//this.filter();
	...
	this.changepage=function(id){
		...
		var htm = postHtml("kapai.php?method=del"+"&gid="+this.id, {
			id: id
		});
		console.log(htm)
		if (htm == "true") 			{
